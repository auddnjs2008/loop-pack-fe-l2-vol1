...
측정 대상은 기존 `Quality` workflow다. PR에서 `quality` job 하나가 실행되고, Ubuntu runner에서 의존성 설치 후 `pnpm check`를 실행한다.
...
cold는 `Set up Node.js` step에서 pnpm cache가 없다고 나온 실행으로 분류했다.
...
warm은 `Set up Node.js` step에서 pnpm cache hit과 restore가 확인된 실행으로 분류했다.
...
pnpm cache는 `Install dependencies` 시간을 줄이는 데는 효과가 있었다. cold에서는 6~9초였고 warm에서는 2초로 줄었다. 하지만 전체 wall-clock 중앙값은 cold 1m49s, warm 1m50s로 거의 줄지 않았다. 따라서 현재 병목은 의존성 설치가 아니라 `pnpm check` 내부 검증과 Playwright Chromium 설치에 있다.

### Before 결론

현재 workflow는 lint, typecheck, test, build, E2E를 한 job에서 모두 실행한다. pnpm cache hit은 확인됐지만, 전체 실행 시간에는 큰 영향을 주지 않았다. Before 기준에서 줄일 후보는 `Run quality checks` 내부를 쪼개 병렬화할 수 있는지, 또는 Playwright Chromium 설치를 E2E job으로 분리해 필요한 경우에만 실행할 수 있는지다.

다만 1단계 최적화에서는 검증을 제거하지 않고, 같은 검증을 유지한 채 병목만 줄여야 한다. 따라서 다음 변경 후보는 `lint`, `typecheck`, `unit test`, `build/e2e`를 독립 job으로 나눌 수 있는지 확인하는 것이다.
